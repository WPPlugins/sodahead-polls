...
description: Allows you to create and embed a poll into your blog.
version: 2.0.8
author: SodaHead.com
author uri: http://www.sodahead.com/
*/
/**
 *  copyright (c) 2011  sodahead.com
 *
...
function logout(){
  /*
   * Logout the current user by removing the auth key from the table.
   */
  global $wpdb;
...
class Sodahead_Polls extends WP_Widget {
  ...
  function getEmbedCode($poll_id){
    global $sodahead_url;
    return '<div style="display: table; margin: 5px auto; font-family: arial,helvetica,sans-serif; text-align: center; width: 250px;" class="widgetContainer"><div id="flashpoll_placeholder_'. $poll_id .'"></div>
        <script type="text/javascript">
         var flashvars={
           ...
           width:"250",
           poll_id:"'. $poll_id .'",
           pollserver:"partners.' . $sodahead_url .'",
           server:"www.' . $sodahead_url .'"
          };
          var params={
  ...
  function uninstall(){
    /*
     * Handle uninstallation/deactivation of the plugin
     */
    logout();
  }
...
function plugin_js_dependencies(){
...
function plugin_view_js_dependencies(){
  wp_enqueue_script('swfobject');
}

add_action('wp_enqueue_scripts', 'plugin_view_js_dependencies');
add_action('admin_init', 'plugin_js_dependencies');
...
register_deactivation_hook(__FILE__, array('Sodahead_Polls', 'uninstall'));
?>
